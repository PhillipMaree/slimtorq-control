...
def run(
    *,
    ...
    if dt_sim > tau_e / 3.0:
        msg = f"dt_sim ({dt_sim * 1e6:.2f} us) too large for stable discrete PI; require dt_sim <= tau_e/3 = {tau_e / 3.0 * 1e6:.2f} us (tau_e = L_s/R_s = {tau_e * 1e6:.2f} us)."
        raise ValueError(msg)
    if dt_sim > T_pwm / 10.0:
        msg = f"dt_sim ({dt_sim * 1e6:.2f} us) too large to resolve PWM at f_pwm={f_pwm:g} Hz; require dt_sim <= T_pwm/10 = {T_pwm / 10 * 1e6:.2f} us."
        raise ValueError(msg)
    ...
    sim = Simulator(
        ...
        dt_ctrl=dt_ctrl,
        Vdc=Vdc,
        i_q_ref_override=i_q_ref_override,
        i_d_ref_override=i_d_ref_override,
    ...
    print(
        f"  R_s={motor.R_s:.4g} Ohm  L_s={motor.L_s * 1e6:.2f} uH  "
    ...
    print(f"  dt_sim={dt_sim * 1e6:.2f} us ({1.0 / dt_sim / 1e3:.1f} kHz)   dt_ctrl={dt_ctrl * 1e6:.2f} us   horizon={T_horizon * 1e3:.1f} ms")
    ...
    b_str = f"{b_est:.3e} N·m·s/rad" if b_est is not None else f"undetermined (|omega_m|={abs(mean_omega):.2e} too small)"
    print(f"Final  i_d={log['i_d_meas'][-1]: .4f} A   i_q={log['i_q_meas'][-1]: .4f} A   i_q*={log['i_q_ref'][-1]: .4f} A")
    print(f"Final  T_e={log['T_e'][-1]: .4f} N.m   TL*={log['TL_ref'][-1]: .4f} N.m   omega_m={log['omega_m_true'][-1]: .3f} rad/s")
    print(f"Steady-state B_est: {b_str}  (FMU B = 1.0e-5)")

    _write_parquet(
        log, motor=motor, foc=foc, Vdc=Vdc, f_pwm=f_pwm, t_dead=t_dead, pi_mode=pi_mode, b_est=b_est, params_json=params_json, params_hash=params_hash, out_path=out_path
    )
